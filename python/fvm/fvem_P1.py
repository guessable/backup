...
def gradient_basis(x):
    ...
    """
    grad_basis_reference = np.array([[-1,  1, 0],
                                     [-1,  0, 1]])
...
class FVEM_P1(VertexCenteredScheme):
    ...
    def _element_stiffness_matrix(self, fh):
        ...
        for i, vh in enumerate(mesh.fv(fh)):
            ...
            grad_basis = gradient_basis(x)
            
            AK[i] = -(norm_prev+norm_next).T@Lambda@grad_basis
        ...
        for vh in mesh.vertices():
            if not mesh.is_boundary(vh):
                for fh in mesh.vf(vh):
                    ...
                    ua_grad = 1/(2*d.area)*( (ua[idx_v_next]-ua[idx_v_prev])/2*d.norm1 + 
...
if __name__ == '__main__':
    ...
    print('\n'+'='*70)    
    print(f'FVEM-P1  (case: {args.case}, mesh: {args.mesh})\n')     
    print(f"{'hmesh':>11s}{'error_max':>11s}{'error_l2':>11s}{'error_h1':>11s}{'Symmetry':>10}")
    for i in range(n_levels):
        fname = f'meshes/{args.mesh}_{i+1}.obj'
        mesh = readmesh(fname)

        solver = FVEM_P1(problem, mesh)    
        ...
        print(f'{hmeshes[i]:11.3e}{errors_max[i]:11.3e}{errors_l2[i]:11.3e}{errors_h1[i]:11.3e}{symmetry:>10}')  
